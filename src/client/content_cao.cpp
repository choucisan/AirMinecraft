...
void GenericCAO::addToScene(ITextureSource *tsrc, scene::ISceneManager *smgr)
{
	...
	// In drone mode, override player visual to cube with drone texture
	ObjectVisual visual = m_prop.visual;
	if (m_is_local_player && m_client->getCamera()->isDroneMode()) {
		m_prop.visual = visual = OBJECTVISUAL_MESH;
		m_prop.mesh = "drone_tiny.obj";
		m_prop.visual_size = v3f(1, 1, 1);
		m_prop.shaded = false;
		m_prop.glow = -1;
		m_prop.use_texture_alpha = false;
		m_prop.textures.clear();
		m_prop.textures.emplace_back("solid_gray.png");
	}
	...
	case OBJECTVISUAL_UPRIGHT_SPRITE: {
		...
		m_meshnode->forEachMaterial([this] (auto &mat) {
			...
			// Force solid material to avoid transparency issues with drone cube
			if (m_is_local_player && m_client->getCamera()->isDroneMode())
		...
		if (!mesh && m_is_local_player) {
			std::string path = porting::path_share + "/games/devtest/mods/g_drone_model/models/" + m_prop.mesh;
			io::IFileSystem *fs = RenderingEngine::get_raw_device()->getFileSystem();
			io::IReadFile *rfile = fs->createAndOpenFile(path.c_str());
			if (rfile) {
				mesh = m_smgr->getMesh(rfile);
				rfile->drop();
				if (mesh)
					mesh->grab();
			}
...
void GenericCAO::step(float dtime, ClientEnvironment *env)
{
	...
	if (m_is_local_player) {
		...
		// In drone mode, compute pitch and roll from movement direction
		if (m_client->getCamera()->isDroneMode()) {
			const PlayerControl &ctrl = player->getPlayerControl();
			f32 target_pitch = 0.0f;
			f32 target_roll = 0.0f;

			if (ctrl.movement_speed > 0.001f) {
				f32 dir = ctrl.movement_direction * core::DEGTORAD;
				f32 forward = std::cos(dir);   // 1=forward, -1=back
				f32 sideways = -std::sin(dir);  // 1=right, -1=left
				target_pitch = forward * 20.0f;
				target_roll = sideways * 20.0f;
			}

			// R/F keys: accumulate roll over time
			m_drone_roll += ctrl.roll * dtime * 120.0f;

			// Smooth WASD-driven pitch/roll toward target
			f32 smoothing = 0.15f;
			if (ctrl.movement_speed > 0.001f) {
				m_drone_pitch += (target_pitch - m_drone_pitch) * smoothing;
				m_drone_roll  += (target_roll - m_drone_roll) * smoothing;
			} else {
				// Gradually return to level when stopped (but keep manual roll)
				m_drone_pitch += (0.0f - m_drone_pitch) * smoothing;
			}

			m_rotation.X = m_drone_pitch;
			m_rotation.Z = m_drone_roll;
		}
...
void GenericCAO::updateMeshCulling()
{
	...
	const CameraMode cam_mode = m_client->getCamera()->getCameraMode();
	const bool drone_mode = m_client->getCamera()->isDroneMode();

	// Hide model in first-person, show in third-person
	bool hidden = false;
	if (cam_mode == CAMERA_MODE_FIRST) {
		hidden = true;
	} else if (drone_mode) {
		// In drone mode: hide in 5 first-person views, show in third-person
		hidden = (cam_mode != CAMERA_MODE_THIRD);
	}
