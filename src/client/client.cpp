...
scene::IAnimatedMesh* Client::getMesh(const std::string &filename, bool cache)
{
	...
	if (it == m_mesh_data.end()) {
		// Fallback: try to load from disk (for drone model etc.)
		io::IFileSystem *fsys = m_rendering_engine->get_filesystem();
		const char *search_dirs[] = {
			"games/devtest/mods/g_drone_model/models/",
			"games/devtest/mods/gltf/models/",
			nullptr
		};
		for (int i = 0; search_dirs[i]; i++) {
			std::string path = porting::path_share + DIR_DELIM
				+ search_dirs[i] + filename;
			io::IReadFile *rfile = fsys->createAndOpenFile(path.c_str());
			if (rfile) {
				scene::IAnimatedMesh *mesh = m_rendering_engine
					->get_scene_manager()->getMesh(rfile);
				rfile->drop();
				if (mesh) {
					mesh->grab();
					if (!cache)
	...
	// Create the mesh from memory cache
...
void Client::preloadModelFile(const std::string &filename)
{
	// Skip if already loaded
	if (m_mesh_data.find(filename) != m_mesh_data.end())
		return;
	...
	for (int i = 0; search_dirs[i]; i++) {
		...
		io::IReadFile *rfile = fsys->createAndOpenFile(path.c_str());
		if (rfile) {
			long size = rfile->getSize();
			std::string data;
			data.resize(size);
			rfile->read(&data[0], size);
			rfile->drop();
			m_mesh_data[filename] = data;
			infostream << "Client::preloadModelFile: loaded "
				<< filename << " (" << size << " bytes)" << std::endl;
			return;
		}
	}
	errorstream << "Client::preloadModelFile: file not found: "
		<< filename << std::endl;
}
