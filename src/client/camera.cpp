...
Camera::Camera(MapDrawControl &draw_control, Client *client, RenderingEngine *rendering_engine):
	...
	m_last_drone_firstperson_view(CAMERA_MODE_DRONE_FRONT),
...
void Camera::update(LocalPlayer* player, f32 frametime, f32 tool_reload_ratio)
{
	...
	// Accumulate roll for drone mode (6DOF)
	f32 roll = player->getRoll();
	if (isDroneMode()) {
		f32 roll_rate = player->control.roll;
		roll += roll_rate * 90.0f * frametime; // 90 degrees/sec
		// Normalize to [-180, 180]
		while (roll > 180.0f) roll -= 360.0f;
		while (roll < -180.0f) roll += 360.0f;
		player->setRoll(roll);
	} else if (roll != 0.0f) {
		// Reset roll when not in drone mode
		roll = 0.0f;
		player->setRoll(0.0f);
	}
	...
	// Calculate and translate the head SceneNode offsets
	{
		...
		case CAMERA_MODE_THIRD_FRONT:
			...
			break;
		// Drone camera modes: small offset for drone body
		...
		case CAMERA_MODE_DRONE_BOTTOM:
			// Drone camera sits at the drone's position with a slight upward offset
		...
		// In drone mode, apply roll instead of hurt tilt
		f32 head_roll = isDroneMode() ? roll :
			(cameratilt * player->hurt_tilt_strength);
		m_headnode->setRotation(v3f(pitch, 0, head_roll));
	...
	if (m_camera_mode > CAMERA_MODE_FIRST &&
			m_camera_mode < CAMERA_MODE_DRONE_FRONT)
	{
	...
	// Drone camera: rotate view direction based on drone_view_index
	if (m_camera_mode >= CAMERA_MODE_DRONE_FRONT &&
			m_camera_mode <= CAMERA_MODE_DRONE_BOTTOM) {
		switch (m_camera_mode) {
		case CAMERA_MODE_DRONE_FRONT:
			// Default forward view, no rotation needed
			break;
		case CAMERA_MODE_DRONE_BACK:
			// 180 degree rotation around Y axis
			m_camera_direction.X = -m_camera_direction.X;
			m_camera_direction.Z = -m_camera_direction.Z;
			break;
		case CAMERA_MODE_DRONE_LEFT:
			// 90 degree rotation left around Y axis
			{
				f32 tmp = m_camera_direction.X;
				m_camera_direction.X = -m_camera_direction.Z;
				m_camera_direction.Z = tmp;
			}
			break;
		case CAMERA_MODE_DRONE_RIGHT:
			// 90 degree rotation right around Y axis
			{
				f32 tmp = m_camera_direction.X;
				m_camera_direction.X = m_camera_direction.Z;
				m_camera_direction.Z = -tmp;
		...
		case CAMERA_MODE_DRONE_BOTTOM:
			// Look straight down
			m_camera_direction = v3f(0, -1, 0);
			abs_cam_up = v3f(0, 0, -1); // Adjust up vector for looking down
			break;
		default:
			break;
		}
	...
	// Third-person drone view: move camera behind drone
	if (isDroneMode() && m_camera_mode == CAMERA_MODE_THIRD) {
		f32 offset_dist = 5.0f * BS; // 5 nodes behind drone
		v3f offset(0, 2.0f * BS, 0); // Slightly above

		offset.X = m_camera_direction.X * -offset_dist;
		offset.Z = m_camera_direction.Z * -offset_dist;

		// Apply offset to camera position
		m_camera_position.X += offset.X;
		m_camera_position.Y += offset.Y;
		m_camera_position.Z += offset.Z;
	}

	// Apply roll: rotate abs_cam_up around m_camera_direction
	if (roll != 0.0f) {
		f32 roll_rad = roll * core::DEGTORAD;
		f32 cr = std::cos(roll_rad);
		f32 sr = std::sin(roll_rad);
		v3f d = m_camera_direction;
		// Rodrigues' rotation formula: rotate abs_cam_up around axis d
		abs_cam_up = abs_cam_up * cr +
			d.crossProduct(abs_cam_up) * sr +
			d * (d.dotProduct(abs_cam_up) * (1.0f - cr));
	}
...
{
	// If in drone mode, cycle through drone views (all first-person)
	if (isDroneMode()) {
		cycleDroneView();
		return;
	}
...
void Camera::cycleDroneView(bool reverse)
{
	if (!isDroneMode())
		return;

	// Cycle: FRONT -> BACK -> LEFT -> RIGHT -> BOTTOM -> FRONT
	static const CameraMode drone_views[] = {
	...
	for (int i = 0; i < num_views; i++) {
		if (m_camera_mode == drone_views[i]) {
			int next = reverse
				? (i - 1 + num_views) % num_views
				: (i + 1) % num_views;
			m_camera_mode = drone_views[next];
			// Sync view index to player for HUD display
			LocalPlayer *player = m_client->getEnv().getLocalPlayer();
			if (player)
				player->drone_view_index = next;
			return;
		}
	}
	// Fallback
...
void Camera::enterDroneMode()
{
	m_drone_mode_active = true;
	m_camera_mode = CAMERA_MODE_DRONE_FRONT;
}

void Camera::exitDroneMode()
{
	m_drone_mode_active = false;
	m_camera_mode = CAMERA_MODE_FIRST;
}
...
void Camera::toggleThirdPerson()
{
	...
	if (m_camera_mode == CAMERA_MODE_FIRST || m_camera_mode == CAMERA_MODE_THIRD) {
		// We're in first-person or third-person = switching
		if (m_camera_mode == CAMERA_MODE_THIRD) {
			// Switch from third-person back to first-person, restore last drone view
			m_camera_mode = m_last_drone_firstperson_view;
		} else {
			// Switch from first-person (drone view) to third-person
			m_last_drone_firstperson_view = m_camera_mode;
			m_camera_mode = CAMERA_MODE_THIRD;
		}
	} else {
		// We're in a drone view -> switch to third-person
		m_last_drone_firstperson_view = m_camera_mode;
		m_camera_mode = CAMERA_MODE_THIRD;
	}
}

bool Camera::isDroneMode() const
{
	return m_drone_mode_active;
}
