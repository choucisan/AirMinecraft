...
void Game::run()
{
	...
	// Auto-enable drone mode if set from main menu
	if (g_settings->exists("drone_mode_on_start") &&
			g_settings->getBool("drone_mode_on_start")) {
		LocalPlayer *lplayer = client->getEnv().getLocalPlayer();
		if (lplayer) {
			PlayerSettings &psettings = lplayer->getPlayerSettings();
			psettings.drone_mode = true;
			// Enable drone mode: free_move, noclip, drone camera
			g_settings->set("free_move", bool_to_cstr(true));
			g_settings->set("noclip", bool_to_cstr(true));
			camera->enterDroneMode();
			updateCameraMode();
			// Preload drone model file into mesh cache
			client->preloadModelFile("drone_tiny.obj");
			// Force visual refresh for drone model
			GenericCAO *playercao = lplayer->getCAO();
			if (playercao) {
				playercao->expireVisuals();
			}
			// Disable built-in minimap (drone HUD has its own)
		if (mapper)
			if (mapper && mapper->getModeDef().type == MINIMAP_TYPE_OFF)
				mapper->setModeIndex(1); // Ensure surface mode for drone HUD
			// Suppress built-in minimap HUD element (drone HUD draws its own at top-left)
	if (mapper) mapper->m_allow_draw = false;
			// Clear setting so normal game starts don't use drone mode
			g_settings->set("drone_mode_on_start", "false");
		}
	}
	...
	while (m_rendering_engine->run()
		...
		// Handle drone mode toggle from pause menu
		if (g_gamecallback->drone_toggle_requested) {
			g_gamecallback->drone_toggle_requested = false;
			toggleDroneMode();
		}

		// Handle nav dialog request from pause menu
		if (g_gamecallback->nav_dialog_requested) {
			g_gamecallback->nav_dialog_requested = false;
			m_game_formspec.showNavTargetDialog();
		}

		// Handle show large map request from nav dialog
		if (g_gamecallback->show_large_map_requested) {
			g_gamecallback->show_large_map_requested = false;
			DroneHud *hud = DroneHud::getActiveInstance();
			if (hud) {
				hud->showLargeMap();
				// Close the formspec so game resumes (map draws in HUD)
				m_game_formspec.showFormSpec("", "");
			}
		}
...
bool Game::createClient(const GameStartData &start_data)
{
	...
	std::string str = std::string("Air Minecraft") +
			" " + g_version_hash + " [";
...
static const char* droneViewName(CameraMode mode)
{
	switch (mode) {
	case CAMERA_MODE_DRONE_FRONT:  return "Front";
	case CAMERA_MODE_DRONE_BACK:   return "Back";
	case CAMERA_MODE_DRONE_LEFT:   return "Left";
	case CAMERA_MODE_DRONE_RIGHT:  return "Right";
	case CAMERA_MODE_DRONE_BOTTOM: return "Bottom";
...
void Game::processKeyInput()
{
	if (wasKeyDown(KeyType::DROP)) {
		...
		// If large map is showing, close it instead of opening pause menu
		if (camera && camera->isDroneMode()) {
			DroneHud *hud = DroneHud::getActiveInstance();
			if (hud && hud->isShowingLargeMap()) {
				hud->hideLargeMap();
				return;
			}
		}
		...
		updateCameraMode();
	} else if (wasKeyPressed(KeyType::DRONE_VIEW_NEXT)) {
		if (camera->isDroneMode()) {
			camera->cycleDroneView(false);
			updateCameraMode();
			std::string msg = std::string("Drone view: ") +
				droneViewName(camera->getCameraMode());
			m_game_ui->showStatusText(utf8_to_wide(msg));
		}
	} else if (wasKeyPressed(KeyType::DRONE_VIEW_PREV)) {
		if (camera->isDroneMode()) {
			camera->cycleDroneView(true);
			updateCameraMode();
			std::string msg = std::string("Drone view: ") +
				droneViewName(camera->getCameraMode());
			m_game_ui->showStatusText(utf8_to_wide(msg));
		}
	} else if (wasKeyPressed(KeyType::DRONE_TOGGLE_HUD)) {
		if (camera->isDroneMode()) {
			bool show = !g_settings->getBool("drone_show_info_panel");
			g_settings->setBool("drone_show_info_panel", show);
			m_game_ui->showStatusText(utf8_to_wide(
				show ? "Drone HUD shown" : "Drone HUD hidden"));
		}
	} else if (wasKeyPressed(KeyType::DRONE_MAP_ZOOM_IN)) {
		if (camera->isDroneMode()) {
			DroneHud *hud = DroneHud::getActiveInstance();
			if (hud) {
				hud->zoomIn();
				m_game_ui->showStatusText(utf8_to_wide(
					"Map zoom level: " + std::to_string(hud->getMapZoom())));
			}
		}
	} else if (wasKeyPressed(KeyType::DRONE_MAP_ZOOM_OUT)) {
		if (camera->isDroneMode()) {
			DroneHud *hud = DroneHud::getActiveInstance();
			if (hud) {
				hud->zoomOut();
	...
	} else if (wasKeyPressed(KeyType::DRONE_TOGGLE_THIRD_PERSON)) {
		if (camera->isDroneMode()) {
			camera->toggleThirdPerson();
			updateCameraMode();
		}
	} else if (wasKeyPressed(KeyType::DIG)) {
		// Left click: handle large map target setting
		if (camera->isDroneMode()) {
			DroneHud *hud = DroneHud::getActiveInstance();
			if (hud && hud->isShowingLargeMap()) {
				v2s32 mouse_pos = input->getMousePos();
				if (hud->handleLargeMapClick(mouse_pos.X, mouse_pos.Y)) {
					m_game_ui->showStatusText(utf8_to_wide("Navigation target set"));
				}
			}
		}
...
void Game::toggleDroneMode()
{
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	PlayerSettings &settings = player->getPlayerSettings();
	settings.drone_mode = !settings.drone_mode;

	if (settings.drone_mode) {
		// Enable drone mode: enable free_move and noclip, set drone camera
		g_settings->set("free_move", bool_to_cstr(true));
		g_settings->set("noclip", bool_to_cstr(true));
		camera->enterDroneMode();
		updateCameraMode();

		// Activate minimap for drone mode
		if (mapper && g_settings->getBool("enable_minimap")) {
			m_drone_minimap_mode = mapper->getModeIndex();
			if (mapper->getModeDef().type == MINIMAP_TYPE_OFF)
				mapper->setModeIndex(1); // Surface mode
		}

		m_game_ui->showTranslatedStatusText("Drone mode enabled");
	} else {
		// Disable drone mode: restore normal camera
		camera->exitDroneMode();
		updateCameraMode();

		// Restore minimap mode
		if (mapper && g_settings->getBool("enable_minimap"))
			mapper->setModeIndex(m_drone_minimap_mode);

		m_game_ui->showTranslatedStatusText("Drone mode disabled");
	}
}
...
void Game::updateCameraDirection(CameraOrientation *cam, float dtime)
{
	...
	if (cur_control) {
		bool large_map = camera && camera->isDroneMode() &&
			DroneHud::getActiveInstance() && DroneHud::getActiveInstance()->isShowingLargeMap();
		cur_control->setRelativeMode(!g_touchcontrols && !isMenuActive() && !large_map);
	}
...
{
	// Skip when large map is shown (user needs cursor for clicking)
	if (camera && camera->isDroneMode() &&
			DroneHud::getActiveInstance() && DroneHud::getActiveInstance()->isShowingLargeMap())
		return;
	...
	if (g_touchcontrols) {
		...
		if (m_invert_mouse || camera->getCameraMode() == CAMERA_MODE_THIRD_FRONT ||
				camera->getCameraMode() == CAMERA_MODE_DRONE_BACK) {
			dist.Y = -dist.Y;
		}
...
void Game::updatePlayerControl(const CameraOrientation &cam)
{
	...
	// Drone roll input: R = roll left, F = roll right
	float roll_rate = 0.0f;
	if (camera->isDroneMode()) {
		if (isKeyDown(KeyType::DRONE_ROLL_LEFT))
			roll_rate -= 1.0f;
		if (isKeyDown(KeyType::DRONE_ROLL_RIGHT))
			roll_rate += 1.0f;
	}
...
{
	// Drone mode: skip damage effects
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	if (player && player->getPlayerSettings().drone_mode)
		return;
...
void Game::processPlayerInteraction(f32 dtime, bool show_hud)
{
	...
	// Block interaction when large map is shown
	if (camera && camera->isDroneMode() &&
			DroneHud::getActiveInstance() && DroneHud::getActiveInstance()->isShowingLargeMap())
		return;
	...
	*/
	...
	case CAMERA_MODE_THIRD_FRONT:
		...
		d = 0;
		break;
	case CAMERA_MODE_DRONE_FRONT:
	case CAMERA_MODE_DRONE_BACK:
	case CAMERA_MODE_DRONE_LEFT:
	case CAMERA_MODE_DRONE_RIGHT:
	case CAMERA_MODE_DRONE_BOTTOM:
		// Drone: shoot from camera position
		shootline.start = camera->getPosition();
		break;
	}
