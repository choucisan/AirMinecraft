...
public:
	...
	// Cycle through drone camera views (front -> back -> left -> right -> bottom)
	void cycleDroneView(bool reverse = false);

	// Enter drone mode (sets camera to DRONE_FRONT)
	void enterDroneMode();

	// Exit drone mode (restores to CAMERA_MODE_FIRST)
	void exitDroneMode();

	// Toggle between first-person and third-person in drone mode
	void toggleThirdPerson();

	// Check if currently in any drone camera mode
...
private:
	...
	bool m_drone_mode_active = false; // Whether drone mode is active (independent of camera mode)
	CameraMode m_last_drone_firstperson_view; // Saved drone view when in third-person
