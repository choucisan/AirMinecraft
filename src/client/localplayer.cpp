		...
		std::vector<CollisionInfo> *collision_info)
{
	...
	*/
	...
	// Drone mode: no gravity, use noclip-style movement
	v3f accel_f(0, -gravity, 0);
	if (player_settings.drone_mode) {
		accel_f = v3f(0, 0, 0); // No gravity for drone
	}
...
void LocalPlayer::applyControl(float dtime, Environment *env)
{
	...
	// Drone mode: full 3D free flight, no gravity, direct velocity control
	if (player_settings.drone_mode) {
		// Drone speed: fast_move speed for normal, walk speed for precise
		const f32 drone_speed = movement_speed_fast * physics_override.speed_fast;
		const f32 drone_speed_slow = movement_speed_walk * physics_override.speed_walk;

		// Horizontal movement direction from yaw
		v3f drone_dir(std::sin(control.movement_direction), 0.0f,
				std::cos(control.movement_direction));
		drone_dir = drone_dir.normalize();

		// Determine speed (aux1 = precision/slow mode)
		f32 speed = control.aux1 ? drone_speed_slow : drone_speed;

		v3f drone_speed_vec = drone_dir * speed * control.movement_speed;

		// Vertical: jump = up, sneak = down
		if (control.jump)
			drone_speed_vec.Y = speed;
		else if (control.sneak)
			drone_speed_vec.Y = -speed;
		else
			drone_speed_vec.Y = 0.0f;

		// Direct velocity control for drone (high acceleration for responsiveness)
		f32 accel = movement_acceleration_fast * physics_override.acceleration_fast * BS * dtime;
		accelerate(drone_speed_vec, accel, accel, true);
		return;
	}
