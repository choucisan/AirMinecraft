...
struct HardcodedPauseFormspecHandler : public TextDest
{
	...
	void gotText(const StringMap &fields)
	{
		...
		if (fields.find("btn_nav_target") != fields.end()) {
			g_gamecallback->showNavDialog();
			return;
		}
...
struct NavTargetFormspecHandler : public TextDest
{
	NavTargetFormspecHandler()
	{
		m_formname = "MT_NAV_TARGET";
	}

	void gotText(const StringMap &fields)
	{
		if (fields.find("btn_show_map") != fields.end()) {
			g_gamecallback->showLargeMap();
			return;
		}

		if (fields.find("btn_set_nav") != fields.end()) {
			DroneHud *hud = DroneHud::getActiveInstance();
			if (hud) {
				// User enters node coordinates, convert to world (BS) coordinates
				f32 x = 0, y = 0, z = 0;
				auto it = fields.find("nav_x");
				if (it != fields.end())
					x = std::strtof(it->second.c_str(), nullptr);
				it = fields.find("nav_y");
				if (it != fields.end())
					y = std::strtof(it->second.c_str(), nullptr);
				it = fields.find("nav_z");
				if (it != fields.end())
					z = std::strtof(it->second.c_str(), nullptr);
				hud->setNavTarget(v3f(x * BS, y * BS, z * BS));
			}
		}
		if (fields.find("btn_clear_nav") != fields.end()) {
			DroneHud *hud = DroneHud::getActiveInstance();
			if (hud)
				hud->clearNavTarget();
		}
...
#define SIZE_TAG "size[11,7.5,true]" // Fixed size (ignored in touchscreen mode)
...
void GameFormSpec::showPauseMenu()
{
	...
	os		<< "button[4," << (ypos++) << ";3,0.5;btn_nav_target;"
		// TRANSLATORS: Pause menu button for setting navigation target
		<< strgettext("Set Nav Target") << "]";
	...
	os		<< "textarea[0.4,0.25;3.9,6.25;;" << "Air Minecraft " VERSION_STRING "\n"
		<< "\n"
...
void GameFormSpec::showNavTargetDialog()
{
	DroneHud *hud = DroneHud::getActiveInstance();

	std::ostringstream os;
	os << "formspec_version[1]" << SIZE_TAG
		<< "label[0.5,0.5;" << strgettext("Set Navigation Target (Drone Mode)") << "]"
		<< "field[0.5,1.5;3,0.8;nav_x;" << strgettext("X") << ";"
		<< (hud && hud->hasNavTarget() ? std::to_string(hud->getNavTarget().X / BS) : "0") << "]"
		<< "field[0.5,2.5;3,0.8;nav_y;" << strgettext("Y") << ";"
		<< (hud && hud->hasNavTarget() ? std::to_string(hud->getNavTarget().Y / BS) : "0") << "]"
		<< "field[0.5,3.5;3,0.8;nav_z;" << strgettext("Z") << ";"
		<< (hud && hud->hasNavTarget() ? std::to_string(hud->getNavTarget().Z / BS) : "0") << "]"
		<< "button_exit[0.5,4.5;3,0.8;btn_set_nav;" << strgettext("Set Target") << "]"
		<< "button_exit[0.5,5.5;3,0.8;btn_clear_nav;" << strgettext("Clear Target") << "]"
		<< "button[4,1.5;3,0.8;btn_show_map;" << strgettext("Show Map") << "]"
		<< "textarea[4,2.5;3,3;;" << strgettext("Click 'Show Map' to open\na large map where you can\nclick to set the target.\n\nOr enter coordinates\non the left.") << "]";

	std::string formspec = os.str();
	FormspecFormSource *fs_src = new FormspecFormSource(formspec);
	NavTargetFormspecHandler *txt_dst = new NavTargetFormspecHandler();

	GUIFormSpecMenu::create(m_formspec, m_client, m_rendering_engine->get_gui_env(),
			&m_input->joystick, fs_src, txt_dst, m_client->getFormspecPrepend(),
			m_client->getSoundManager());
	m_formspec->setName("MT_NAV_TARGET");
	m_formspec->doPause = true;
}
